...
export const createCheckoutSession = async ({ configId }: { configId: string }) => {
  ...
  const existingOrder = await db.order.findFirst({ where: { userId: user.id, configurationId: configuration.id } })
  ...
  if (existingOrder) {
    ...
    order = await db.order.create({ data: { amount: price / 100, userId: user.id, configurationId: configuration.id } })
  }
  ...
  const stripeSession = await stripe.checkout.sessions.create({
    ...
    mode: "payment",
    shipping_address_collection: { allowed_countries: ["RO", "US"] },
    metadata: {
      userId: user.id,
      orderId: order.id
    },
